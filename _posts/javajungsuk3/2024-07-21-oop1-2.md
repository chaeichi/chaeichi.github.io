...
excerpt_image: /assets/images/excerpt-images/javajungsuk3/2024-07-21-oop1-2.png
categories: 
    - Java
tags: 
    - Java
    ...
    - 클래스
---
![banner](/assets/images/excerpt-images/javajungsuk3/2024-07-21-oop1-2.png)
...
- 객체 배열
    ...
    - 객체 배열 안에는 객체가 아닌 `객체의 주소`가 저장된다.
    - 다른 배열과 동일하게 객체 배열도 같은 타입의 객체만 저장할 수 있다.
- 데이터와 함수의 결합
    - 데이터 저장개념의 발전과정
...
- 제어자와 메서드를 이용한 제약조건 추가
    - 객체지향언어에서는 제어자와 메서드를 이용하여 데이터에 추가적인 제약조건을 코드에 반영하는 것도 가능
    - `제어자`를 이용해서 변수의 값을 직접 변경하지 못하도록 함
    - `메서드`를 통해서 값을 변경할 때 지정된 값의 유효성을 조건문으로 점검한 다음 유효한 값일 경우에만 변경하도록 함